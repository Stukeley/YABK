...
func int C_WantToCallGuards(var C_Npc slf)
{
	...
	{
		if((slf.guild == GIL_PAL) || (slf.guild == GIL_MIL) || (slf.guild == GIL_VLK) || (slf.guild == GIL_SLD) || (slf.guild == GIL_DJG) || (slf.guild == GIL_BAU))
		{
		};
	};
	return FALSE;
...
func void B_CallGuards()
{
	...
	{
		if((self.guild == GIL_MIL) || (self.guild == GIL_PAL) || (self.guild == GIL_SLD) || (self.guild == GIL_DJG))
		{
			B_Say_Overlay(self,other,"$ALARM");
