...
func int ZS_Attack_Loop()
{
	...
	if(Hlp_IsValidNpc(other) && (C_NpcIsDown(other) == FALSE))
	{
		...
		self.aivar[AIV_LASTTARGET] = Hlp_GetInstanceID(other);
		if(self.aivar[76] != Npc_GetDistToNpc(self,other))
		{
			Npc_SetStateTime(self,0);
			self.aivar[76] = Npc_GetDistToNpc(self,other);
		};
		waffe = Npc_GetReadiedWeapon(other);
		other.aivar[AIV_MagicUser] = MAGIC_ALWAYS;
		Hlp_IsValidItem(waffe);
		if((Npc_GetStateTime(self) > 5) && (self.aivar[76] == Npc_GetDistToNpc(self,other)) && (self.aivar[76] >= 200) && Hlp_IsValidItem(waffe))
		{
			other.aivar[AIV_MagicUser] = MAGIC_ALWAYS;
			if(waffe.flags & (ITEM_CROSSBOW == ITEM_CROSSBOW))
			{
				Npc_ClearAIQueue(self);
				AI_SetWalkMode(self,NPC_RUN);
